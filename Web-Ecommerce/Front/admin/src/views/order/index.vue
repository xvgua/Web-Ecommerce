  ...
  <div class="order-manage">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <el-input v-model="keyword" placeholder="搜索订单号" clearable style="width: 200px" @keyup.enter="handleSearch" />
        <el-select v-model="status" placeholder="订单状态" clearable style="width: 140px" @change="handleSearch">
    ...
    <el-table :data="orders" v-loading="loading" border>
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="ORDER_STATUS_COLOR[row.status]">{{ row.statusText }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 1" text type="success" @click="handleShip(row.id)">发货</el-button>
          <el-button v-if="row.status === 0 || row.status === 1" text type="danger" @click="handleCancel(row.id)">取消</el-button>
        </template>
      </el-table-column>
    </el-table>
