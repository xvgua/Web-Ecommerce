...
│  │  [立即支付]                                     │ │ │  ────────────          │
│  └─────────────────────────────────────────────────┘ │ │  应付金额 ¥XXX.XX      │
│  ┌─ 阶段二：扫码支付（QR Code + 自动扫码）───────────┐ │ │  [立即支付/我已完成支付]│
│  └─────────────────────────────────────────────────┘ │ └────────────────────────┘
│  ┌─ 商品清单 ──────────────────────────────────────┐ │
│  │  商品图片 | 商品名 | 单价 | 数量 | 小计            │ │
│  └─────────────────────────────────────────────────┘ │
...
确认订单 (/order/confirm)
  → 提交订单 (POST /api/orders)
    → 跳转支付页面 (/orders/:id/pay)
      → 选择支付方式
        → 点击"立即支付" (POST /api/orders/:id/pay-intent)
          → 展示二维码（canvas，qrcode 库生成）
            → 3秒后自动模拟扫码 (POST /api/orders/:id/scan-simulate)
              → 点击"我已完成支付" (PUT /api/orders/:id/pay/confirm)
...
2. **支付方式必选** — 创建支付会话时 payMethod 必传
3. **防重复提交** — 按钮点击后进入 loading 态，防止重复请求
4. **状态校验** — 进入支付页时检查订单状态，非待支付状态自动跳转订单详情页并提示
5. **扫码前置** — "我已完成支付"按钮仅在扫码完成后可点击
6. **二维码生成** — 使用 `qrcode` 库（`QRCode.toCanvas()`），内容为 `{ orderNo, token, amount, method }` JSON
7. **自动扫码** — QR 展示 3 秒后自动调用 `POST /scan-simulate` 模拟用户扫码
8. **响应式布局** — 桌面端（≥768px）左右分栏，移动端（<768px）上下堆叠
9. **倒计时提示** — 页面顶部展示"订单将在30分钟内自动取消"提示
...
### 12.1 收藏页面（`views/user/favorites.vue`）

**路由：** `/user/favorites`（需登录）

**页面职责：**

- 展示当前用户收藏的商品列表（商品图片、名称、价格、库存状态）
- 支持商品卡片点击跳转商品详情页
- 每项收藏提供"加入购物车"和"取消收藏"操作
- 空数据时展示 `el-empty` 引导用户浏览商品
...
│  │  商品图 | 商品名 | 价格 | [加入购物车] [取消]   │ │
│  └──────────────────────────────────────────────┘ │
│  （空时显示 el-empty "暂无收藏，去逛逛吧"）         │
└───────────────────────────────────────────────────┘
```

### 12.2 收藏入口

1. **商品详情页** — 价格区域旁增加收藏按钮（心形图标 `Star` / `StarFilled`），已收藏显示实心红色，未收藏显示空心
2. **导航栏** — 用户下拉菜单或顶部导航增加"我的收藏"入口 → `/user/favorites`

### 12.3 收藏交互流程

```
商品详情页 → 点击收藏按钮
  ├─ 已收藏 → DELETE /api/favorites/:productId → 取消收藏（图标变空心）
  └─ 未收藏 → POST /api/favorites { productId } → 添加收藏（图标变实心红色）

收藏列表页 → 点击商品 → 跳转商品详情页
收藏列表页 → 加入购物车 → 调用购物车 API → toast 提示
收藏列表页 → 取消收藏 → 二次确认 → DELETE → 刷新列表
```

### 12.4 收藏约束

1. **登录校验** — 未登录点击收藏跳转登录页，使用 `useAuth` composable 或路由守卫
2. **收藏状态同步** — 进入商品详情页时调用 `GET /favorites/:productId` 检查收藏状态，显示对应图标
3. **乐观更新（推荐）** — 点击收藏/取消收藏后立即切换 UI 状态，请求失败时回滚；简化实现亦可走 `try { await api; 更新状态 } finally {}` 模式
4. **操作反馈** — 收藏/取消时 toast 提示（"已收藏"/"已取消收藏"），加入购物车同购物车模块反馈
5. **二次确认** — 仅收藏列表页取消收藏需 `ElMessageBox.confirm`（列表页为删除操作，需确认；详情页为即时 toggle，无需确认）
6. **空状态** — 收藏列表为空时 `el-empty` + "去逛逛"按钮跳转商品列表
7. **下架商品** — 已下架商品在收藏列表中仍展示，但"加入购物车"按钮置灰并提示"已下架"
8. **响应式** — 桌面端列表布局，移动端（<768px）卡片堆叠
9. **商品名称脱敏** — 无需脱敏，正常展示
10. **批量操作** — 不做批量取消收藏

### 12.5 导航栏入口

在 `DefaultLayout.vue` 顶部导航用户菜单中增加"我的收藏"入口：
