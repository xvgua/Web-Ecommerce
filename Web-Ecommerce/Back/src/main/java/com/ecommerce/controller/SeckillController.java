...
public class SeckillController {
    ...
    public Result<List<Long>> getMyPurchased() {
        Long userId = UserContext.getUserId();
        if (userId == null) return Result.success(List.of());
        return Result.success(seckillService.getUserPurchasedProductIds(userId));
    }

    @PostMapping("/order")
    public Result<Order> createOrder(@Valid @RequestBody SeckillOrderRequest req) {
        Long userId = UserContext.getUserId();
        return Result.success(seckillService.createSeckillOrder(userId, req));
    }
